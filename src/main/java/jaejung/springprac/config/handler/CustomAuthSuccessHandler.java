package jaejung.springprac.config.handler;

import jaejung.springprac.domain.user.UserDetailsDto;
...
/**
 * 사용자의 ‘인증’에 대해 성공하였을 경우
 * 수행되는 Handler로 성공에 대한 사용자에게 반환값을 구성하여 전달합니다
 */
@Slf4j
@Configuration
public class CustomAuthSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    private JSONObject convertObjectToJsonObject(UserDto userDto){
        JSONObject json = new JSONObject();
        json.put("userName", userDto.getUserName());
        json.put("userEmail", userDto.getEmail());
        return json;
    }
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        JSONObject userVoObj = convertObjectToJsonObject(userDto);
