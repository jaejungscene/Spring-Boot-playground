...
import jaejung.springprac.config.filter.CustomAuthenticationFilter;
import jaejung.springprac.config.handler.CustomAuthFailureHandler;
import jaejung.springprac.config.handler.CustomAuthSuccessHandler;
import jaejung.springprac.config.handler.CustomAuthenticationProvider;
...
public class WebSecurityConfig {
    /**
     * 1. 정적 자원(Resource)에 대해서 인증된 사용자가
     * 정적 자원의 접근에 대해 ‘인가’에 대한 설정을 담당하는 메서드이다.
     *
    ...
    /**
     * 2. HTTP에 대해서 ‘인증’과 ‘인가’를 담당하는 메서드이며
     * 필터를 통해 인증 방식과 인증 절차에 대한 등록 및 설정을 manage한다.
     *
